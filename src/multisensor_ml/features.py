...
def build_causal_features(
    ...
    *,
    ...
    for factor in factors:
        ...
        output[z_name] = robust_z.astype("float32")
        names.append(z_name)
        for window in windows:
            ...
            output[mean_name] = rolling.mean().astype("float32")
            output[std_name] = rolling.std(ddof=0).fillna(0).astype("float32")
    ...
    for name, values in (
        ...
        names.append(name)
    output["is_awake"] = frame["is_awake"].astype("int8")
    names.append("is_awake")
    for context in CONTEXT_CATEGORIES:
        name = f"context__{context}"
        output[name] = (frame["context"] == context).astype("int8")
        names.append(name)
    return output, names
