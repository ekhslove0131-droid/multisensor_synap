...
model_handle="bjcoding/multisensor-goal15-availability"
...
model_url="https://www.kaggle.com/models/bjcoding/multisensor-goal15-availability"
multisensor-ml() {
  "${repo_root}/.venv/bin/multisensor-ml" "$@"
}

multisensor-ml availability-model verify --package "${package_root}" >/dev/null
...
readback_root="$(mktemp -d /private/tmp/multisensor-availability-readback.XXXXXX)"
status=""
...
download_and_verify() {
  ...
  remote_root="$(dirname "${manifest}")"
  if [[ -f "${remote_root}/model_payload.tar.gz" ]]; then
    if ! multisensor-ml availability-model verify --package "${remote_root}" >/dev/null; then
      echo "remote packaged archive failed local verification" >&2
      return 1
    fi
  elif [[ -d "${remote_root}/model_payload" ]]; then
    # Kaggle's model download --untar expands model_payload.tar.gz and does
    # not retain the archive. Verify the expanded payload hashes and sample
    # predictions against the immutable manifest instead.
    if ! "${repo_root}/.venv/bin/python" - "${remote_root}" <<'PY'
import json
import pathlib
import sys
...
outer = json.loads((root / "model_manifest.json").read_text())
payload = root / "model_payload"
...
for profile_id in outer["profile_ids"]:
    package = load_availability_variant(payload / "profiles" / profile_id)
...
PY
    then
      echo "remote expanded model payload verification failed" >&2
      return 1
    fi
  else
    echo "remote readback has neither archive nor expanded model_payload" >&2
    return 1
  fi
  printf '%s\n' "${manifest}"
}
...
if [[ "${variation_exists}" == false ]]; then
  ...
  if ! current_manifest="$(download_and_verify "${current_version}")"; then
    echo "existing Kaggle version failed readback verification" >&2
    exit 1
  fi
  ...
  if [[ "${remote_hash}" == "${local_hash}" ]]; then
    ...
    kaggle models variations versions create "${remote_variation_handle}" \
      -p "${package_root}" -n "Verified sensor-availability oracle/sanity candidate" -r skip
    status="VERSIONED"
  fi
fi
...
if [[ "${status}" != "REUSED" ]]; then
  ...
  if ! remote_manifest="$(download_and_verify "${version}")"; then
    echo "new Kaggle version failed readback verification" >&2
    exit 1
  fi
fi
remote_hash="$(${repo_root}/.venv/bin/python -c \
